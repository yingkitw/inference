...
pub fn get_model_path() -> Option<PathBuf> {
    std::env::var("INFLUENCE_MODEL_PATH")
        .ok()
        .map(PathBuf::from)
}

pub fn get_temperature() -> f32 {
    std::env::var("INFLUENCE_TEMPERATURE")
        .ok()
...
pub fn get_top_k() -> Option<usize> {
    std::env::var("INFLUENCE_TOP_K")
        .ok()
        .and_then(|v| v.parse().ok())
}

pub fn get_repeat_penalty() -> f32 {
    std::env::var("INFLUENCE_REPEAT_PENALTY")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(1.1)
}
...
pub fn get_max_tokens() -> usize {
    std::env::var("INFLUENCE_MAX_TOKENS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(512)
}
...
pub fn get_device() -> String {
    std::env::var("INFLUENCE_DEVICE")
        .unwrap_or_else(|_| "auto".to_string())
}

pub fn get_device_index() -> usize {
    std::env::var("INFLUENCE_DEVICE_INDEX")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(0)
}
