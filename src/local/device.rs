...
pub fn get_device(pref: DevicePreference, index: usize) -> Result<Device> {
    match pref {
        ...
        DevicePreference::Metal => {
            #[cfg(feature = "metal")]
            {
                let device = Device::new_metal(index)
                    .map_err(|e| InfluenceError::LocalModelError(format!("Metal GPU not available: {}", e)))?;
        ...
        DevicePreference::Cuda => {
            #[cfg(feature = "cuda")]
            {
                let device = Device::new_cuda(index)
                    .map_err(|e| InfluenceError::LocalModelError(format!("CUDA GPU not available: {}", e)))?;
                info!("Using CUDA GPU");
                return Ok(device);
            }
...
mod tests {
    ...
    fn test_device_preference_from_str() {
        ...
        assert!(matches!(DevicePreference::from_str("auto"), Ok(DevicePreference::Auto)));
        assert!(matches!(DevicePreference::from_str("cpu"), Ok(DevicePreference::Cpu)));
        assert!(matches!(DevicePreference::from_str("metal"), Ok(DevicePreference::Metal)));
        assert!(matches!(DevicePreference::from_str("cuda"), Ok(DevicePreference::Cuda)));
        assert!(matches!(DevicePreference::from_str("AUTO"), Ok(DevicePreference::Auto)));
        assert!(matches!(DevicePreference::from_str("CPU"), Ok(DevicePreference::Cpu)));
