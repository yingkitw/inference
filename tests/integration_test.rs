...
async fn test_v1_generate_endpoint() {
    ...
    match response {
        ...
        Err(e) => {
            if e.is_connect() {
                eprintln!("Server not running. Start with: influence serve --model-path <path>");
...
async fn test_v1_generate_stream_endpoint() {
    let url = "http://localhost:3000/v1/generate_stream?prompt=Hello&max_tokens=20&temperature=0.7";
